...
from ds2hf_pipeline import hf_upload
from ds2hf_pipeline.tcga import (
...
from ds2hf_pipeline.tcga import webdataset as wds_mod
from ds2hf_pipeline.tcga.gdc import GDC_BASE_URL, GDCClient, write_cases_json
from ds2hf_pipeline.tcga.gdc import eq as gdc_eq
...
def _write_ssgsea_stats(processed_dir: Path, only: set[str] | None) -> None:
    ...
    from ds2hf.tcga.schema import SSGSEA_COLLECTIONS, TABULAR_TABLES, ssgsea_stats_table
...
def fetch_pathology_reports_cmd(
    ...
    `ds2hf_pipeline.tcga.pathology` for why no text extraction happens here.
    """
...
def build_cmd(
    ...
    for path in project_files:
        ...
        from ds2hf.tcga.schema import SSGSEA_COLLECTIONS, ssgsea_patient_column
...
def build_gene_expression_quantification_cmd(
    ...
    typer.echo("upload with: ds2hf-pipeline tcga upload-gene-expression-quantification")
...
def build_cdr_cmd(
    ...
    typer.echo(f"\nwrote dataset card -> {card}")
    typer.echo("verify with: ds2hf-pipeline tcga verify-cdr")
