...
**The [`ds2hf.mskchord`][loader] loader nests every config under its patient locally.** Sharing that nested table would share a modified version, so this repository ships the code to build it, not the table:

```python
# pip install "ds2hf @ git+{GITHUB}#subdirectory=packages/ds2hf"
from ds2hf.mskchord import Chord
...
```python
from ds2hf.mskchord import cna_long  # see "One row per patient"
