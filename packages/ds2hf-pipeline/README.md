...
Build pipelines for the ds2hf HuggingFace datasets, one sub-command group
per source:

```
ds2hf-pipeline tcga <command>       # TCGA from the NCI Genomic Data Commons
ds2hf-pipeline mskchord <command>   # MSK-CHORD from cBioPortal
```

For the read side alone, install the companion [`ds2hf`](../ds2hf/)
package instead.
...
Only the `upload*` commands need auth. Put a
...
Builds the [`gabrielaltay/tcga-patients-open`][patients] and
[`gabrielaltay/tcga-tabular-open`][tabular] datasets, and their per-project
companions: fetches open-access TCGA data from the National Cancer
Institute (NCI) Genomic Data Commons (GDC), flattens it into per-project
Parquets, and pushes the result to HF Hub.
...
```
ds2hf-pipeline tcga fetch-clinical           # GDC -> raw cases.json per project
ds2hf-pipeline tcga fetch-mutations          # GDC -> Masked Somatic Mutation MAFs from WXS (DNA)
ds2hf-pipeline tcga fetch-expression         # GDC -> Gene Expression Quantification TSVs from RNA-Seq (STAR counts)
ds2hf-pipeline tcga fetch-pathology-reports  # GDC -> Pathology Report PDFs (scanned BCR documents)
ds2hf-pipeline tcga fetch-msigdb             # Broad -> md5-pinned MSigDB gene sets (GMT) for ssGSEA
ds2hf-pipeline tcga fetch-copy-number        # GDC -> every open-access copy number file (4 data types, 6 workflows)
ds2hf-pipeline tcga fetch-methylation        # GDC -> SeSAMe methylation beta values (Illumina arrays)
ds2hf-pipeline tcga fetch-mirna-isoform      # GDC -> miRNA isoform-level quantification
ds2hf-pipeline tcga build                    # raw -> consolidated per-project parquets + dataset card
ds2hf-pipeline tcga build-tabular            # raw -> per-(project, table) parquets + dataset card
ds2hf-pipeline tcga build-project-tabular    # raw -> ONE project's standalone tabular dataset + card
ds2hf-pipeline tcga build-webdataset         # raw -> per-patient WebDataset tar shards + index.parquet
ds2hf-pipeline tcga upload                   # push processed/ to the consolidated HF dataset repo
ds2hf-pipeline tcga upload-tabular           # push processed_tabular/ to the tabular HF dataset repo
ds2hf-pipeline tcga upload-project-tabular   # push one project's tree to tcga-<slug>-tabular-open
```
...
`ds2hf-pipeline tcga <cmd> --help` for arguments. Notable flags:
...
Builds `gabrielaltay/msk-chord-2024`: the MSK-CHORD cBioPortal study
(Jee et al., *Nature* 2024), converted from tab-separated text to parquet
and otherwise unchanged. The read side is `ds2hf.mskchord`.
...
per-patient nesting live in the `ds2hf.mskchord` loader, which runs on the user's machine,
where the license (section 2(a)(1)(B)) allows them.

The conversion rules are in `mskchord/convert.py`'s docstring, and the
dataset card lists them as the license's required notice of changes.

### Commands

```
ds2hf-pipeline mskchord fetch    # cBioPortal -> <data-dir>/raw/msk_chord_2024.tar.gz (sha256-pinned)
ds2hf-pipeline mskchord build    # tarball -> <data-dir>/processed/ (parquets, tarball, LICENSE, card)
ds2hf-pipeline mskchord verify   # every cell vs the tarball; the tarball vs cBioPortal and datahub
ds2hf-pipeline mskchord upload   # verify, then push processed/ (private unless --public)
```
...
`mskchord/source.py` to the new tarball and the datahub commit it matches,
